					...
					for (int face = 0; face < 6; face++)
					{
						...
						if (std::find(finded.begin(), finded.end(), false) == finded.end()) 
						{
							...
							for (int i = 0; i < 6; i++) 
							{
								...
								vec3 p1_� = computational_mesh.nodes[computational_mesh.elems[computational_mesh.elem_shifts[elem_id] + tmp[0]]];
								vec3 p2_� = computational_mesh.nodes[computational_mesh.elems[computational_mesh.elem_shifts[elem_id] + tmp[1]]];
								vec3 p3_� = computational_mesh.nodes[computational_mesh.elems[computational_mesh.elem_shifts[elem_id] + tmp[2]]];
								vec3 p4_� = computational_mesh.nodes[computational_mesh.elems[computational_mesh.elem_shifts[elem_id] + tmp[3]]];
								
								std::vector<vec3> p_c = { p1_�, p2_�, p3_�, p4_� };
